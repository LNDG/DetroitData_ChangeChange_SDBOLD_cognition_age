...
% 28-10-20
    ...
    if exist(fname, 'file') >= 1
	    ...
	    %if strfind (fname, '.nii.gz') >= 1 % techinically this searches anywhere in the filename, so if someone named their file something strange such as 'object_filt.nii.gz_2mm_MNI.nii', it'll also accept this as a zipped file, which is of course untrue
	    if strcmp (fname(end-6:end), '.nii.gz') == 1
